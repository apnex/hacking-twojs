...
function toAnchors(path, curve = 1) {
	let anchors = [];
	path.forEach((p) => {
		if(anchors.length == 0) {
			p.command = 'M';
		...
		switch(p.command) {
			case('M'):
				anchors.push(new Two.Anchor(p.x, p.y, 0, 0, 0, 0, Two.Commands.move));
			break;
			case('L'):
				anchors.push(new Two.Anchor(p.x, p.y, 0, 0, 0, 0, Two.Commands.line));
			break;
			case('Q'):
				if(curve) {
					anchors.push(new Two.Anchor(p.x, p.y, p.controls.left.x, p.controls.left.y, p.controls.right.x, p.controls.right.y, Two.Commands.curve));
				} else {
					anchors.push(new Two.Anchor(p.x, p.y, p.controls.left.x, p.controls.left.y, p.controls.right.x, p.controls.right.y, Two.Commands.line));
				}
			break;
			case('Z'):
				anchors.push(new Two.Anchor(p.x, p.y, 0, 0, 0, 0, Two.Commands.close));
			break;
			default:
				anchors.push(new Two.Anchor(p.x, p.y, 0, 0, 0, 0, Two.Commands.line));
		}
...
// construct a polygon
function makeShape(mx = 10, my = 10, radius = 20, sides = 6, rotation) {
	let points = [];
	if(typeof(rotation) === 'undefined') {
		rotation = (360 / sides) / 2;
	}
